...
- **常设原则**:
  ...
  - **additive-only**:不改/不删 core;新评分维度叠在 dual-score 上。
...
**linchpin = T1(EX grader)**——使 35.9pp 可量化,是 R14/G3/R17/G9/G10/R21 的校准 oracle 与客观 GT 来源。
...
`packages/eval/eval/cases/rbi-10000251-exec/`(39 case,GA-EVAL-REAL-EXEC 12.8% 真执行基线所依赖)**未被 git 追踪**,源文件在仓外 `/Users/mckenzie/workspace/reverse-bi/eval-cases/10000251/`;`git clean` 会清掉基线。根目录另有未追踪 `analyze-real-exec-gap.mjs`。待决定去向(纳入 git 或文档化外部依赖)。
