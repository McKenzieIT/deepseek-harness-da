...
日期：2026-09-08 · 当时意图是取代 v1 笔记并将其隔离为 `_superseded/…SUSPECT.md`

> **编辑注（2026-09-08 合并时补）**：那次隔离**未被采纳**。v1 笔记仍在 [`exec-grader-papers.md`](exec-grader-papers.md)，因为 v3 用 PDF 原文证实了它的论文层两条头条；本文与 v1、[v3](exec-grader-papers-v3.md) 并存，三方对账在 v3 §7。另需注意本文 §0 的自述：它**读过 v1 的 R1 Answer 与笔记全文**，故非盲测；v3 的仓库层另派了两个不知情 subagent 取证。
