...
2. **判官必须拿到参考答案** —— `2608.17938` Arm 2 是唯一直接测过「无参考」代价的实验：ICC `0.888 → 0.628`、分数**通胀 +0.074**、只能靠答案核对的题判别力掉到 `30–36%`。且本仓 39 个 case 的 `expected.sql` 正被 loader 丢弃 ⇒ **与 [T11](T11-loader-source-strip.md) 是同一块工作，不是新方向。**
