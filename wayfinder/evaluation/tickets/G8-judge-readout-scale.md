...
2. **参考答案的形态**：`expected.sql` 文本、执行结果集，还是两者？（**与 [G1b](G1b-ground-truth-lifecycle.md) 的 source-evidence 决议耦合 —— 若 G1b 已定，本条直接引用，不重裁。**）
...
5. ~~证据落盘~~ —— **不由本票裁**：judge 的 `schema_context` / prompt 变体 / 量表版本是否入 artifact（现状 80 份结果文件 **0 份**记录）。[T1](T1-exec-grader-impl.md) 已关闭且只实现 execution evidence；G8 实施前须为 judge evidence 独立建票。
...
2. **判官必须拿到参考答案** —— `2608.17938` Arm 2 是唯一直接测过「无参考」代价的实验：ICC `0.888 → 0.628`、分数**通胀 +0.074**、只能靠答案核对的题判别力掉到 `30–36%`。[T11](T11-loader-source-strip.md) 已让本仓 39 个 case 的 `expected.sql` 可达；G8 只决定如何消费经过 G1b 验证的参考。
...
- 参考答案的 source-evidence lifecycle（[G1b](G1b-ground-truth-lifecycle.md)）。
