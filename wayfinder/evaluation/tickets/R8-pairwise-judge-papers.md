...
## 补记（2026-09-11）：一处负空间结论被自己的补搜证伪

本票原 Resolution 第 3 条曾据「读过的六篇都不研究二值 + 阈值化聚合」推出「整片文献无人研究」。**这是从样本到全称的跳跃，已被证伪**——详见 [research note §9](../research/pairwise-judge-papers.md#9-2026-09-11-文献补搜r8-的一处负空间结论被证伪)。

- **[GEAR（2606.03361）](https://arxiv.org/abs/2606.03361) 正做此事**：准则为隐 Bernoulli（全程二值），baseline 即 **flat 聚合 vs 确定性 gating** 头对头；把我们叫「漏」的量就叫 **leakage**，失效模式命名为 **False Credit Propagation**；报 leakage 削减 96.5%，且明确测了 gating 的**代价**。
- 另两条削弱：`2510.11822`（minority-veto；判官 TPR 96% / **TNR<25%**）、`2606.00093`（命名 item-level aggregation 的 **decision rule**，换池化口径即把准确率 0.551→0.899）。
- **仍站得住且更窄**：HealthBench（`2505.08775`）与 Autorubric（`2603.00077`）这两个二值 rubric 的实际参照点**都没有阈值**，所以「单 prompt 内二值准则的固定 k-of-n 截断」在文献里**没被辩护也没被攻击**；`2603.28005` 是唯一 holistic vs 原子分解头对头（holistic 三胜二），但两者是**独立条件**，从未让一个推翻另一个。
- **改后的声明**：无人测量过「单个判官 prompt 内 holistic 准则被同场机械子准则投票推翻」，更无人在**生产落盘判决**上测过；本仓的 8.56pp 漏与 52.03% 是自有贡献，**GEAR 的 leakage 是表达它的现成量纲**。

**对 G8 是净利好**：第 1 决策（读出形状）从「无先例、自己发明」降级为「在已发表方案中选并说明差异」。

**证据等级**：以上为**摘要层 + 权威元数据**（16/16 ID 验真、标题逐字核对），**未达本票对那六篇的全文认读标准** ⇒ 不得直接进决策，须由 **R8b** 全文认读后才可被 G8 引用。R8b 重点两问：① GEAR 的 leakage 定义能否逐字套用于本仓 1495 条判决；② 其 prerequisite 图必须人写还是可归纳（若必须人写，本仓 5 维的图就是 G8 要裁的东西之一）。
