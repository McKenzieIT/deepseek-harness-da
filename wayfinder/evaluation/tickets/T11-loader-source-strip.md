...
**Blocked by**: 无
**Unblocked**: [T1 — Execution grader implementation](T1-exec-grader-impl.md)（resolved 2026-09-12）、[G1b — Ground-truth lifecycle](G1b-ground-truth-lifecycle.md)
...
`EvalCaseSchema` 的 zod object 默认 strip 未知键，导致 case 文件里已有的 source 字段在加载时被静默丢弃：`expected.sql`（reference SQL）、`meta`（含 `anchor_ds` 快照锚点、`tier`、`source`）、`schema_version`。且 `expected.sql` 是**模板**而非可直接执行的 SQL（含 `{{ds_yesterday}}` 等占位符），其解析依赖同一 case 的 `meta.anchor_ds`。
...
`rbi-10000251-exec` 的 case 形如（`eval_10000251_036.yaml`）：`schema_version: 3`、`expected.sql`（人写的 reference SQL）、`expected.behavior`、`meta.anchor_ds: "20260806"`、`meta.tier: verified`、`meta.source: migrated`。
...
2. **G1b 以为要新建的 schema 已经存在。** R1 的「0 个 case 带 reference SQL」只对 `k11-v2` 成立；`rbi-10000251-exec` 的 39 个已带 rbi `schema_version: 3` 的 source。G1b 的迁移分类应以「保留既有 schema 还是合流」为起点，而非从零设计。
...
4. **通用缺陷**：只要 strip 行为不变，**今后往 case 文件加的任何 source 字段都会被静默忽略**，且没有任何信号。这比丢掉当前三个字段更严重。
...
- **D5 把「loader 不得静默吐掉未知 `expected.*` 字段」划归 G1 所有权**（理由是 fail-loud 与 source 内容无关）。本票是这条决议的**实现票**。
...
- `loadCase` 保留 `expected.sql`、`meta.anchor_ds`、`meta.tier`、`meta.source`、`schema_version`（或显式声明的等价字段），并有测试逐字段 pin 一个带这些字段的 fixture 往返不丢。
...
**loader 侧**（`packages/eval/eval/src/eval_case.ts`）—— rbi 字段声明为 optional，两套 case set 过同一个 schema；`schema_version`、`expected.sql`、`expected.behavior`、`meta.anchor_ds`、`meta.tier`、`meta.source` 均存活，逐字段有 fixture 往返测试。`case_loader.ts` 本体未动——丢弃发生在 schema，不在 loader。
...
- **`meta` 用 `looseObject`：未声明键保留。** 与 `dimensions` 已有的 record 形状对称。grader 要读的三个（`anchor_ds`/`tier`/`source`）声明并定型，其余 source（`roles`、`needs_repin`、`business_context` …）原样存活。**代价**：这些字段无类型。接受，因为它们是语料元数据而非判分输入；且本票验收要的“今后加字段不再被静默忽略”在这两个位置得到满足。
- **全仓 `passthrough` 被否决**：它保住字段但全部无类型，且写错的结构键仍然无信号——把本票要消除的静默又买了回来。
- **全仓 `strict` 被否决**：会把 `meta` 下每个未列举的 source 变成加载错误，使语料无处放自由形式溯源。
...
主分支同步将语料元数据键统一为 `meta.source`，并将持久化证据类型统一为 `CaseSource` / `caseSource`；T11 的 strict structural fields、loose `meta` / `dimensions` 和 reference-SQL 模板解析行为保持不变。RBI fixture 与 loader tests 已随字段名更新。
