...
**Type**: grilling  ·  **Status**: open · ready
**Part of**: [dsh-data-agent evaluation map](../map.md)
**Completed prerequisites**: [R1 — 执行级评分与非循环 ground truth 论文认读](R1-exec-grader-papers.md)、[T11 — case loader 静默丢弃 reference SQL 与 snapshot 锚点](T11-loader-source-strip.md)、[T1 — Execution grader 实现](T1-exec-grader-impl.md)
**Implementation boundary**: T1 只把 case 自带的 source evidence 原样记录，不解释 `anchor_ds` 是否有效锚点，也不回填 expected；这些 lifecycle 决策仍归本票。
...
本票需要与人共同决定作者与 reviewer 独立性、snapshot identity、artifact source evidence、变更与退役规则、多个可接受 artifact 的表达、动态 case 或 relation assertion 的准入条件，以及 143 个 legacy/unverified expectation 与 25 个 delivery-only case 的迁移分类。SQL 的实际执行复用 dsh-data-agent 的 `@deepseek-ai/dsh-query` capability；evaluation 只拥有 ground-truth source evidence、benchmark versioning、policy reference 和证据语义。
...
**本票原假设「source-evidence schema 待设计」是错的——它已经存在。** 实测：
...
`rbi-10000251-exec` 的 case 带 rbi `schema_version: 3` 的完整 source evidence：`expected.sql`（人写的 reference SQL）、`meta.anchor_ds`、`meta.tier: verified`、`meta.source: migrated`。R1 的「0 个 case 有 reference SQL」**只对 k11-v2 成立**。

三处后果，本票必须据此重写起点：

1. **迁移分类的起点是「保留既有 rbi schema 还是与 k11-v2 合流」，不是从零设计。** 两套 schema 如何合流本身归 [G10](G10-harness-bhe-split.md)；本票只定 lifecycle 语义。
2. **[T11](T11-loader-source-strip.md) 已让这些字段在 eval 路径可达。** G1b 可以直接基于 39 个现有 `CaseSource` 记录裁定 lifecycle，不再等待 loader 修复。
