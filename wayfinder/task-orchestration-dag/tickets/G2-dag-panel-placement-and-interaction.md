...
**Current standing**: interaction goals and prototype evidence remain; [G15 Current client placement](G15-current-client-placement.md) owns production slots. The Client consumes Task DAG Remote values resolved through Host Bindings, not a `DagModelService`, raw DSH Session events, or a `taskGraph` SessionProjection. The historical terminal-state, `dag_task_*`, and Todo-successor context below is superseded by G14 and [G16 Model tools and cross-preset composition](G16-todo-coexistence-and-preset-composition.md).
