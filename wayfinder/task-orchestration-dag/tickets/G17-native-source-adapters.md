...
[G13 ExecutionAttempt and correlation protocol](G13-task-work-correlation.md) fixes the adapter contract direction: every ordinary Attempt has one primary Binding; nested tool, subagent, workflow, and external-effect work uses child Bindings; skill use is method metadata rather than a fabricated run lifecycle. Adapters receive Host-created context, persist a prepared Binding before dispatch when the native id is not yet known, bind the returned native id afterward, submit all observations and proposals through Host commands, and never infer authority from trace or lineage. A native execution has one causal owner, while reusable results cross Tasks through OutputRefs.
