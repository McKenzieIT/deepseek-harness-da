...
The first-release baseline from the outer-loop decision is fixed: a Plan-DAG-enabled profile disables upstream `goal-round-driver`; the Task DAG driver is the sole cross-turn continuation owner; phase policies remain inside an Attempt; ordinary DSH profiles retain the upstream Goal driver unchanged. This follow-up may reopen Goal/Plan integration only after first-release evidence identifies a real user benefit and second continuation-policy consumer. It must not silently remount two automatic drivers. Any coexistence requires an explicit ownership protocol, measured ROI, profile migration, and tests for inbox competition, pause/resume, revision fencing, budgets, Holds, and Stop Reasons.
