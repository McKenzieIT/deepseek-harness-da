...
Parity has separate static and dynamic checks. Stage 0 mounts all three presets and proves that their complete registered catalogues contain exactly these 15 tools. During Stage 1 and Stage 2, policy and floor must expose that same full catalogue; every state-machine `request/header` catalogue may contain only names from it. The state-machine Session union may omit tools from phases the case never reaches, including an INTERPRETATION phase skipped by a correct persistent-failure decline.
...
**Stage 1 — real smoke.** Six approved cases — `g25a_exec_037`, `g25a_exec_039`, `g25a_exec_046`, `g25a_exec_042`, `g25a_exec_048`, and `g25a_fail_01` — run once per arm for 18 Attempts. This is the approved 3×L2 + 2×L3 + one persistent-failure mix, covering every complexity level present in the verified slice. Stop before the decision batch if policy and floor do not expose the same full tool catalogue, if state-machine exposes a tool outside that catalogue, if working-set digests differ across arms, if a successful query has no readable outcome, if the absolute dates do not reach the first model request, if a reference SQL disagrees with its case expectation, if any arm's infrastructure failure rate exceeds 5%, or if the scorer can mark a confident business conclusion correct with no successful query. Smoke validates the protocol only and is excluded from the effect statistics.
