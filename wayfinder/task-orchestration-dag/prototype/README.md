...
**演示动效(2026-09-02 定稿,像素级验证)**:D 默认**自动演进**(⏸/⏵⏵,2.6s/步,
...
流动节奏**恒定周期/秒**(与 zoom 无关,缩放后无需重装):初版"屏幕像素速度恒定
48px/s"在低 zoom 下变成 ~10 周期/秒 的抖动(人眼对节奏敏感,流动隐喻惯例 1–3 cyc/s),
定稿 **派生边 2 cyc/s、包含边 1 cyc/s**(层级:主旋律/低音),呼吸 0.67 cyc/s:

- **派生边流动**(紫虚线,目标运行中)+ **包含边缓动**(琥珀虚线,工作流运行中);
...
- **详情卡取消**:点节点显示,再点画布空白处即收起(A/C/D 全部容器)。
