...
### Results (judge-only re-baseline — IN PROGRESS at commit time)

`node --import tsx/esm packages/eval/eval-cli/src/bin.ts --cases packages/eval/eval/cases/rbi-10000251-exec --pass-k 3 --concurrency 3 --today 20260806 --scope-id 10000251 --provider aga --model qwen3.7-max --skip-health-gate --output /tmp/dsh-eval-d-judgeonly/ --run-id ga-eval-retry-feedback-judgeonly-20260906`. ~11/39 at commit. **Pre-fix (post-prompt-fix) judge-only = 22/39=56.4% pass, null-SQL 22/117=18.8%, non-SQL 0%.** Numbers + verdict to amend when complete — target null-SQL 22→<12, pass_rate >56.4% (or honest report if (d)'s benefit is bounded by event-case null-SQLs needing (a)).
...
3. **Criteria #1 (null-SQL drop) + #2 (pass_rate): PENDING the judge-only baseline** (real-exec baseline also needed — deferred; run when concurrent session idle + network restored, like GA-EVAL-SQLGEN-PROMPT-FIX).
4. **Honest caveat**: (d)'s measured benefit is bounded — the null-SQL explosion is dominated by EVENT cases (125/127/129/130) where the model declines for lack of the event schema (the (a) problem); (d)'s feedback may not flip those. (d) WILL help fixable-critic-error cases (the DWS 038-060 cluster). The baseline shows the real split.

### Pointer

[GA-EVAL-RETRY-FEEDBACK](../tickets/phase-misc/GA-EVAL-RETRY-FEEDBACK-wiring-gap.md) — code + tests landed (commit `ba1b1ed597`; backup `backup-ga-eval-retry-feedback`). Judge-only baseline running → numbers to amend. **Follow-up**: (1) amend this entry with judge-only numbers (read `/tmp/dsh-eval-d-judgeonly/ga-eval-retry-feedback-judgeonly-20260906.json` — null-SQL + pass_rate vs 56.4%/22); (2) run real-exec baseline (--with-query + MAXC_CONFIG + maxc-sidecar-k11.mjs) when concurrent idle + network restored; (3) resolve (a) GA-EVAL-EVENTDEF-PREFETCH (LLM-detection or description-mining — risk-gate verdict on its ticket).
