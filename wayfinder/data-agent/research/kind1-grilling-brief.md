...
**What survives — every §2/§3 argument.** The narrow-scope rejection (§2a) rests on the *existence* of a pointer from Chinese rule bodies to a boilerplate header; that coupling is **real**, only the citations were wrong. §3's "≈82% of the English already exists, ≈18% stale in exactly the GA-GT2 dimension" verdict **is correct — but only by coincidence, and it is now independently re-derived**. §3 computed it as `~146 of 808 live buildPrompt CJK`; both of those numbers belong to the falsified family, but they were inflated proportionally so the ratio survived. Verified derivation: the three GT2-changed sites are `prompt.ts:54` (rule 1, 50 CJK), `:56` (rule 3, 23 CJK) and `:145` (date block, 52 CJK) = **125 stale**; the buildPrompt side is 782 total − 85 in the `buildEvalPrompt` region = **697**; 125/697 = **17.9% stale → 82.1% reusable**. The three stale sites in the English arm are confirmed present at `exp2-prompts-en.ts:75`, `:77`, `:89`. §1's 0.2% ratio is unaffected (817 vs 440,988 ≈ 0.19%). The §2 "13.6% boilerplate" share was computed on the falsified 1161 denominator and has **not** been recomputed — treat it as "small, unquantified".
