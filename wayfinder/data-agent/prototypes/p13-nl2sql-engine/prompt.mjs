// P13 prototype — SQL 生成 prompt（移植 RBI v2-baseline.md）。
// §3 staged SOP（A 准备/B 生成/C 校验/D 执行）+ §6 八规则 + §5 诚实拒答 + 工具目录映射
// harness tool + MAX_SQL_PER_TURN=8 + 适配 P7 四阶段 preset（GENERATION 生成 SQL）。
// 方言 grounding（key_differences/functions/cast_map/sql_templates）从 conventions seam 注入。
// prompt 本身是文本（TS 移植成本低，research §2 组件 5）。

import { MAX_SQL_PER_TURN, MAX_FEEDBACK_RETRIES } from './types.mjs';
import { renderConventionsPrompt } from './conventions.mjs';

// 工具目录（映射 RBI v2-baseline §1 → da harness tool seam）
const TOOL_CATALOG = `# 工具集（da harness tool seam 映射）
- search_data_sources(query): BM25 schema-linking 检索返候选数据源（P13 bm25-linking 经 P5 ctx.retrieval seam）
...
- check_query(instance_id): 续取运行中查询（P4 ctx.query）
- critique_sql_tool(sql, question): pre-exec critic（P13 critic 填 P7 sql_syntax_gate 槽）
...
export function buildPrompt({ question, candidates, eventDef, conventions, phase = 'generation' }) {
  const dialect = renderConventionsPrompt(conventions);
  const candLines =
    candidates && candidates.length
      ? candidates.map((c) => `- ${c.id}: ${c.payload?.description || c.id} (score=${Number(c.score).toFixed(3)})`).join('\n')
      : '（无候选）';
