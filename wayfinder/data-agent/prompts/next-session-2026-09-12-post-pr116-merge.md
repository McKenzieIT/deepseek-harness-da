...
- **worktree**：剩 7（master + 2 eval 不碰 + resync + dsh-rda-admin + 2 p2-keep）；`dsh-arch` 已定向 rescue + 删（`62c09c1d5d` master design doc + `7f618cfe40` resync manifest）。
...
## 二、剩余 worktree 决策（1 项：dsh-rda-admin，需用户拍板；dsh-arch 已 rescue+删）

1. ~~**`dsh-arch` 定向 rescue**~~ ✓ DONE（`62c09c1d5d` master design doc 纠正注 + `7f618cfe40` resync manifest excluded + `git branch -D chore/um-arch-impl-2026-09-08` + worktree remove）。代码已 absorbed via arch-regen `038d8b51ce`（in origin/master）；3 处残留已保 2（design doc + manifest），ticket Cross-check 跳过（451a788901 摘要够用）。
