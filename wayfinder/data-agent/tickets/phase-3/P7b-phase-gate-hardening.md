...
**Type**: prototype
**Phase**: 3
**Status**: Blocked by P13（GENERATION critic——生产 preset 接线；P7 prototype stub 了 sqlglot）
...
**前置/衔接**：P13（GENERATION critic——sqlglot AST + JSON-path + registry，P7 stub 了）= 主 blocker；其余 data 插件包（P4 `query-tool` consumer Not-yet-specified / P5 / P9 / P10）随各自 ticket 发包后接入 preset 行。persona 拆 `dsh-data-persona` 包（P7 D2 留口）。G1（Pipeline vs goal/todo）随 P7 已解锁，可平行实验对比。
