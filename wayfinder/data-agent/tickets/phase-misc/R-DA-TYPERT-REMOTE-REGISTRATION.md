...
- **2026-09-11（主 session — shard 2 ui-settings-models）**：ui-settings-models 39 → 0（tsc client 187 → 148，`--force` 权威 + lefthook 绿）。commit `03e865a148` on resync `[Follow-on-3-B] ui-settings-models: strip ghost types + D3 migration`。三类 driver 全触及：ghost `IApiClient`/`CredentialView`/`ConfigurableProviderView`/`DiscoveredModelView` → `ClientRemote`/`CredentialInfo`/`LlmConfigurableProvider`/`LlmDiscoveredModel`（`@deepseek-ai/dsh-api-remotes/client`）；D3 positional args（`settings.mutate(ns,ops,rev)`/`credentials.describe(refs)`/`credentials.set(ref,val)`/`credentials.unset(ref)`/`llm.discoverModels(ns,req,signal?)`/`llm.providers({})` → `llm.listConfigurableProviders()`）；`RemoteResult<T>` 无 `.result` wrapper（`.ok/value/error` 直接）；`.value.{providers,credentials,models}` sub-key gone；event `'credentials/updated'` → `'credentials/reference-updated'`；test bench `new TestRemote(ctx).emit(event, args)` 替 `$dispatch`。**D1（`LlmConfigurableProvider` 无 `active` field）**：cross-ref `ctx.remote.llm.listProviders()` 折 `row.active` 上 `ProviderRow`（hard-fail on either half，mirror listConfigurableProviders 失败处理；文档理由 in store.ts:156-158）。`providerUsable`/`onboardingReadiness` 读 `row.active`。stale README 提 `ConfigurableProviderView`（EN+ZH）= 文档 drift，非阻塞 tsc，follow-up。

- **2026-09-11（主 session — shard 3-b result-cache）**：result-cache 4 → 0（tsc client 148 → 144，`--force` 权威 + lefthook oxlint/whitespace 全绿 + vitest 30/30 green，3 spec files）。commit `d0f152ba32` on resync `[Follow-on-3-B] result-cache: strip ghost IApiClient + D3 migration to inject=[remote]`。**D2 = A（wrapped，cache.ts 契约保留）**：generated client type `result/get` = `(resultId: ResultId) => Promise<RemoteResult<ResultEntry>>`（`packages/data/result-cache/lib/typert.remote-client.d.ts`——`TypertRemoteNamespace$726573756c74`）——`RemoteResult` 与 `RpcResult` 结构一致（`{ok:true;value:T} \| {ok:false;error}`），cache.ts 内单行 barrel swap `RpcResult` → `RemoteResult`（同 `@deepseek-ai/dsh-api-remotes/client` barrel line 138）即替代所谓「stale RpcResult type」；ResultFetcher 契约字面不变（`Promise<RemoteResult<ResultEntry>>`），epoch guard / single-flight / LRU / session-isolation 全保。Typert `RemoteResult` docstring 明示 discourage B：*"the Client face rebuilds an instance for the RemoteResult error branch, so `throw result.error` keeps throw semantics… carrier failures folded into the error branch, so no consumer wraps a call to recover one; only assembly faults still reject"*——B unwrap-rewrap 即撤销 carrier 已做的工作 = fork drift。**D3 = `inject=['remote']`**：service.ts fetch() 直调 `this.ctx.remote.result.get(resultId)`；index.ts 移除 `ConnectionHandle` import + `connection.api` 拼装 + `ResultServiceConfig.api` field；`ctx.on('connection/reset', ...)` 保留（plain cordis event bus，非 `connection` service inject——同 `packages/client/ui-settings-plugins/src/client/index.ts` 模式，实证）。**types.ts readonly-tighten** `rows: unknown[][]` → `readonly (readonly unknown[])[]` 让 canonical `RemoteResult<CanonicalResultEntry>`（`rows: readonly (readonly Json[])[]`）无 cast 直接可赋——同时对齐模块 docstring「entries treated as immutable read-only views」。**RemoteFailure 是 `RemoteError<Code>` 实例 union 非 plain object**（`packages/typert/protocol/src/types.ts:60` + `remote-error.ts`）——`'result-not-found'` 是 declared code（`packages/data/result-cache/src/types.ts:48` declare-merge 到 `RemoteErrorDetailsMap` with `{resultId}` details）；`'gateway/internal'` 是 base declared code。tests: `TestRemote(ctx, {result: {get}})` 替 `makeApi(IApiClient)`；`new RemoteError('result-not-found', 'miss', {resultId})` / `new RemoteError('gateway/internal', 'boom', {})` 替 plain-object error branches；wire assertion `toHaveBeenCalledWith('qr_1')`（positional，无 signal——host `get(resultId): ResultEntry` 无 signal 参）替 `toHaveBeenCalledWith({resultId}, undefined)`；`inject` assertion `['remote']` 替 `['connection']`；`runtime.ctx.provide('connection', {api} as never)` gone。cache.ts 内 `_signal` 保留在 fetcher 签名以维护 in-flight epoch guard，wire 层 no-op（host 契约无 signal）。**发现 shard 2 vitest 债（本 shard 顺便验，非本 shard 引入）**：ui-settings-models tsc-clean at `03e865a148` 但 vitest = 122 failed / 100 passed / 6 spec files（`store.client.spec.ts` / `provider-form.client.spec.tsx` / `welcome-store.client.spec.ts` / `welcome-notice.client.spec.tsx` / `apply.client.spec.ts` / `readiness.client.spec.ts`）。Failure signature = `status: 'error'` 期望 `'ready'` + `Cannot read properties of undefined (reading 'credentials')`——一致 hypothesis：shard 2 D1 decision（`ctx.remote.llm.listConfigurableProviders()` + `listProviders()` 双调 fold `row.active`，hard-fail-on-either-half）在 test bench 只 mock 单调时 throw。非本 shard 引入（本 shard 未 touch ui-settings-models 任何文件；shard 2 tsc-绿 但 vitest 未跑，本 shard 首次跑发现债）。落 follow-up 票 `R-DA-UI-SETTINGS-MODELS-VITEST-DEBT`。本 shard 目标「148 → 144」+「无 vitest 回归 (result-cache)」双达标——回滚原则不适用（shard 2 legacy 非本 shard 回归；rollback 破坏 39 tsc 进度且不修 ui-settings-models）。

## Resolution

(open — B shard 3-b of 4 done. Remaining B: Phase-2 blocked by Plan B ADR-0002 = client/runtime 131 delete + 4 presenter 13 migration。shard 4 = 小包 17 也归 Phase-2。)
