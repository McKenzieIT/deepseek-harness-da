...
- **load_* code-review fix**（2026-08-21 本会话）：处理 540dbd155d ship 后两 subagent code review findings（3 MAJOR + MINOR/NIT，无 CRITICAL）。MAJOR-1 白名单 overclaim → **加 `GENERATION_TOOLS` 白名单**（additive；`phase-gate/src/types.ts` 加 `load_table_definition`/`load_event_definition`——GENERATION 写 SQL 前 schema grounding 合理 + critic harvest `partition_cols`/`event_params` 一致），使 README/package.json/src tool description/preset 行内注释的 "UNDERSTANDING/GENERATION" 全部 accurate（零 doc 改）；phase-gate 加 GENERATION guard 测试。MAJOR-2 substrate try/catch → 两 `load*DefinitionResult` 包 `schema.load_*` 于 try/catch（substrate `load_*` 是 strict `Schema.parse`-on-match，name 匹配但 schema 失败抛 ZodError + readdir/readFile I/O 错），catch 返 `{found:false, message:'substrate error: <sanitized>'}`（单行/去路径/截断 200，不泄 raw stack）+ malformed-fixture 测试（name 匹配但 schema 失败 → found:false 不抛）。MAJOR-3 preset header stale → 重写 `agent.cordis.yml` 两块 header 注释（4 工具已 ship+uncommented；仅 critique/evaluate/present_* 仍注释占位）。MINOR：table 投影 `domains`（对齐 event）+ `engine` render（grounding-relevant）+ 两包完整 nested `output.schema`（消除 `as unknown as TableModel/EventModel` cast + output-validation 强制投影形状）；disambiguation/caliber_variants/primary_key_unique 决策**补否**（YAGNI for SQL grounding）。NIT：empty-type 去尾随空格、not-found `JSON.stringify(name)` 对齐 invalid-name、found:true 无 table 中性 fallback、name 长度上限 200、empty-string record key filter。验证：tsc -b + vitest（phase-gate 18 + table 17 + event 17 = 52/52）+ verify-cordis-config（135）+ gate oxlint（`.oxlintrc.staged.json` 非类型感知，0 error）+ verify-translation-pairing（本包绿；pre-existing rescope-fork-codemod + bundle/concurrent tickets 失败非本会话回归）全绿。**ctx.schema bundle mount 仍 deferred**（并发活跃——cd2b741409 phase-gate reasoningEffort + D2e，正改 map/AGENTS/preset/tickets；pnpm-lock 撞车风险，待并发空闲）。〔resolving ticket：`tickets/phase-2/P6b-semantic-layer-hardening.md`（review-fix note appended）〕

- **load_* review-fix-2**（2026-08-21 本会话）：两 subagent review（正确性 + 镜像/清理 lens）判 **ship-ready**（无 CRITICAL/MAJOR must-fix）。修：types.ts MAJOR-1 注释去 harvest 过度引用（harvest 实为 pre-existing 非功能，见下"浮出"）+ sanitizer 路径脱敏正则改进（覆盖相对路径，不仅绝对）+ 孤儿 JSDoc 重排（sanitizer 移到 pure-load-core JSDoc 前，恢复 doc 绑定）+ I/O-error/路径脱敏测试（table S18）+ event metrics empty-key 测试（event S18，补 S15 交叉覆盖）；tsc + vitest 55/55 + gate oxlint 0 全绿。**浮出（不修——phase-gate/concurrent 域）**：harvest 集成 bug——`captureToolData` 探 top-level 但 load_* 返 `{found,table|event:{…}}` 嵌套 → critic partition_cols/event_params 捕不到；pre-existing（540dbd155d ship 形 mismatch）+ grounding-relevant（critic GENERATION 门 partition/param 检查缺数据），待 phase-gate owner 修（concurrent 正改 phase-gate.ts）。**留（可辩护 YAGNI，两 subagent 一致）**：table kind-for-dws 不单独渲染（仅 dim 内联）/ disambiguation(term_defaults)·primary_key_unique·caliber_variants 不投影（substrate 默认空/nullable 低信号；pk_unique 对 join cardinality 边界相关——填充后再议）。**记录修正**：GENERATION-guard 测试本会话所写但并发 `git add` 扫入 cd2b741409（该处 RED——测试在、whitelist 未加）；本 review-fix（a127875845）加 whitelist 使其 GREEN——故"加 GENERATION guard 测试"应读作"测试在 cd2b741409、whitelist 在本 review-fix"。
- **load_* harvest bug fixed**（2026-08-21, ticket A — [`critic-harvest-load-nesting`](critic-harvest-load-nesting.md) resolved）：`phase-gate` `captureToolData` 探嵌套 `value.table`/`value.event`（非 top-level）+ `collectFields` 增 array-of-objects `name` 叶提取（load_* 返 projected `[{name,…}]`，substrate map→array；ticket 的 nesting-only snippet 不足——TDD 证 nesting-only 仍 RED）。critic 的 `partition_cols`/`event_params` 现从 load_* 真填充。tsc + vitest(20/20) + gate-oxlint + verify-cordis-config(135) 全绿。commit 同时 land 了 orphaned GENERATION-guard 测试（MAJOR-1 whitelist 配对）。
