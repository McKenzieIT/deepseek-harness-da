...
**carry-forward（Phase 5+，已实施 2026-09-02）**：
- #25 nested scopeId propagation in code-mode `run_code` 子调度——**已实施**（commit `dd05d43bb3`）：code-mode.ts:479 加 `...exec.scopeId !== undefined ? { scopeId: exec.scopeId } : {}`（conditional spread，parity rootCallId/agent/parent，exactOptionalPropertyTypes 合规）；`ToolExecutionInput.scopeId` JSDoc 更新为"已传播 nested via code-mode run_code sub-dispatch"（之前"Phase 5 才传播"）；2 回归测试（传播 + dormant）。
- #37 harness-responder.ts D3ii——**已实施**（commit `e42f30e64f`）：`HarnessBootOptions` 加 `scopeId?`；bootContext :253 `'k11'` → 经 `semanticLayerConfig()` seam 用 `this.opts.scopeId`（hardcoded 'k11' 消除，仅 JSDoc 引用残留）；D3ii throw（seam 在 `new Context`/plugin mount 前 fast-fail "explicit scopeId required (D3ii)"）；main.ts:278 传 `args.scopeId`；2 测试（positive + D3ii throw）；EXP2_ARM 未碰（git diff context.ts + engine.ts 空）。

**→ GA-GT1-cleanup 前置门**：Phase 3+4（scopeId required + active warn-on-use + 现存 scope 赋正式 default tenant + 删 active/setActive/clearActive + tenant 必填 + 移除 scopeEpoch）= breaking change。**前置条件**：①Phase 1+2 生产验证（多租户并发无泄漏/竞态）+ ②全 callers 迁移（#25 nested + #37 harness-responder carry-forward **已实施** commits `dd05d43bb3` + `e42f30e64f`；剩余 caller 待 Phase 3 scopeId required 时强制）+ ③用户授权 breaking change。满足后开 [GA-GT1-cleanup](GA-GT1-cleanup-multi-tenant-scope.md) 票。
