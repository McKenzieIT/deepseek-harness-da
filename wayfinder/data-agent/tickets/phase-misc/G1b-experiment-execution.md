...
4. **B/C/D preset 不存在 + A 自身是骨架**：`packages/bundle/data-agent/presets/data-agent/agent.cordis.yml` 仅注册 `tool-search-data-sources`；`tool-query-data`/`load_*`/`critique_sql`/`evaluate_sql_quality`/`present_*` 全注释 TBD。故已 resolved 的 A 也跑不完一个 case。
