...
- **A roster 补完**：解注释 `packages/bundle/data-agent/presets/data-agent/agent.cordis.yml` 的 TBD 工具行（`query_data` 待 P4c；`load_*` 待 ctx.schema model-facing tool 包；`present_*` 待 INTERPRETATION delivery ticket；`critique_sql`/`evaluate_sql_quality` 评估是否需独立 tool 行——critic 已 fold 进 phase-gate 单向 delegate）。
- **B/C/D preset**：新 preset 文件（`packages/bundle/data-agent/presets/data-agent/{b-free-react-planning,c-hybrid,d-bare-react}.cordis.yml` 或同文件多 preset，dsh-plugin-development **Mode 6 agent-preset-composition**），复用 A 的 persona+数据工具 roster，仅 toggle phase-gate 行（`cordis:group`+`isolate:phaseGate` realm）+ planning group 行。
