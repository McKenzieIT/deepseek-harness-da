...
1. **keychain provider mount 形态（crux）→ SPEC 到 G3c landing**：
   ...
   - **1b unlockPassword/autoLock = autoLock:300（保 P12b locked-when-idle 收窄）+ unlockPasswordSource: 'interactive'(默认,安全) | 'env'(无值守 fallback,bash 可读弱化锁,诚实文档) | 'none'**。P12b §7.3 secret-to-protect 递归：唯一真正安全=interactive（不落盘/env）；P12c native ACL 才是 runtime-exfil 真修。
