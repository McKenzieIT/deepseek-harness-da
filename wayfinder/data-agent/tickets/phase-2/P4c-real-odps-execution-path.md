...
- **(b) guard chain / (c) tool-query Consumer**：pending（maxc `--cost-check`/`--max-rows`/`--wait`/`job` 提供多数 guard；tool-query Consumer = `query_data` model-facing tool + preset 注册，属 G1c/preset）。
