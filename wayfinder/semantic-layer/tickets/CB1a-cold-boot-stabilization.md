...
- **merge 状态**：PR #11 已 admin-merge 进 master（merge commit `a905858f5d`，2026-09-05；master 无 branch protection → admin-merge 无视 larger-runner pending）。larger-runner 3 job（coverage/static/snapshots）在 fork（User 账户无 larger-runner 权限）→ 维持 `pending`（不红、不阻塞）；ubuntu-latest CI 红的根因（`CLAUDE.md` git mode 误为 symlink 120000 → Linux checkout ENAMETOOLONG）由 [PR #17](https://github.com/McKenzieIT/deepseek-harness-da/pull/17)（`2e487635ce`）单独修复并先合入 master。Release 版本 gate 红（6 包落后 rc.8）由 [PR #18](https://github.com/McKenzieIT/deepseek-harness-da/pull/18)（`0f4bcae7`）对齐修复。**CI 后续状态**（post #17/#18/#19/#21/#25/#27/#28/#29/#31/#33/#11）：**全部已知 CI 红已解**——Release+No-prod-src(#17+#18)+Sandbox cordis 2(#19)+pwsh 3+Python 3(#28 macos-14)+eval-cli(#27+DASHSCOPE secret)+catalog(#29 regen)→ green；#31 修 #19 的 YAML colon bug(Sandbox 0s 挂→能跑)；#33 加 NODE_OPTIONS=4G(seatbelt build:lib:host OOM→不炸)；Pages 启用(Deploy docs configure-pages 404→过)；E2E manual-only(#25)；larger-runner 3 job 跑 ubuntu-latest(#21)。剩:ubuntu-latest 3 heavy job OOM + vitepress build 第二层(等 CI)。
