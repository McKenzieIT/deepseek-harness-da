...
- [W27 — 可扩展 Semantic Graph 投影与 concept 支持](tickets/W27-extensible-semantic-graph-projection.md): 开放 `RelationDef.type`、registry 驱动 `buildGraph` + canonical target mapping、disposer + 缓存失效、prototype-pollution fix、graphDataBridge 迁移至 owner Remote 类型；真实 Remote 证据通过 `graph-remote.spec.ts`（直接调用真实 `@Remote('getGraphData')` 服务方法 — host program，无 TS6307）。
