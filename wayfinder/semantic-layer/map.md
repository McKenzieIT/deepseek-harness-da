...
- **eval pass^k 迁移 —— 离线重打分完成,blast radius 远小于预期**（[重打分报告](research/passk-rescore-2026-09-03.md)）:
  ...
  - **更正(2026-09-04,协议核查):目标值其实**不需要**重设** —— 基线不是一个数,取决于 `passK`。`scripts/run-eval.sh` 的**显式默认就是 `--pass-k 1`**(注释:"baseline-matching flags: `--pass-k 1`"),CL-15 的 73.8% 与 CL-22 的 73.2% 中位数**都是 k=1**;而 pass^k 与 best-of-k 在 k=1 时**数学恒等** → **CL-15 标准基线没失效,今天跑仍是 ~73%**。52.4% 的跌幅来自 **k 从 1 改成 3**,不是判定规则。两者测不同问题:k=1 问"能不能做对",k=3+pass^k 问"是不是**稳定**做对"。故 **CL-20/21/23/R11 的阈值按 k=1 + CL-22 中位数口径原样有效,无需重设、无需重跑基线**(四票已各自追加协议澄清段)。
  - **⚠️ CL-22 的「≥3 run 取中位数」与 pass_k=3 方向相反**:前者把抖动当噪声抹平,后者当失败惩罚。两条规则都用「3」但含义对立,并存时需明确适用范围,否则会出现"3 run 中位数的 pass_k=3 结果"这种双重惩罚口径。
  - **真正待决的是口径问题,不是数字问题**(见 [GA-EVAL-REBASELINE](../data-agent/tickets/phase-misc/GA-EVAL-REBASELINE-passk-semantics.md),已重构前提):要不要把验收切到 k=3+pass^k?若切则所有历史目标值须整体重设(一次口径变更决策,非 bug 修复);若不切则 k=1 基线继续有效、pass^k 作为附加可靠性指标并行观测。另:**pass^k 方差量级未知**且理论上远大于 best-of-k 的 ±2.4pp——若大到无法用中位数判阈值,则问题是"该口径能否用于验收"而非"阈值定多少"。建议先用 30-case ×3 探针量方差(~3h,现有 `a4fbd262`=63.3% 可作第一个 run)。
