...
- [CL-20: DELIVERY Type-2 agent 行为](tickets/CL20-delivery-agent-behavior-type2.md) — **决策已定（D1-D5）**：主观类证伪为 case-set 问题→毕业 CL-25；D 类门禁已实现但**有已知未修缺陷**（`052` 被 3/5 误伤，`"weekly report"` 样例与「N天每天的X」冲突）→ 修 prompt + rebase + 重跑全量后开 PR（**frontier — 无阻塞**）
