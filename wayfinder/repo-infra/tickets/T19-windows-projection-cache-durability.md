...
**Status**: open
**Assignee**: unclaimed
**Related**: [T11](T11-test-coverage-failing.md)；证据取自 `windows node 24 / coverage` job 104534944084（PR #155 head `7f1365dfb6`）

## Question

`packages/session/session-projection-cache` 有三项断言只在 Windows coverage 上失败，Linux 同一提交全绿：
...
三项都在 `vi.waitFor(..., { timeout: 5_000 })` 内轮询读文件。
...
1. 先让失败可归因：在这三项上捕获 `ctx.logger.warn`，断言「无 warn」与「读回到期望值」两件事分开报告。这一步本身就是修复的一部分——fail-soft 写路径缺少可观测性。
