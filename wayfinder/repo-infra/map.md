...
- [T16: duplication 门 89 clones](tickets/T16-duplication-gate-89-clones.md) — jscpd 报 89 clones / exit 1（**pre-existing**，在 `origin/master` 上同样复现，且非本轮新增文件所致）。它在 `lint and duplication` 组里 fail-fast，**掐掉后面 5 道门**（`test:expected` / `web browser snapshot` / `doc-typecheck:contracts-ready` / `node-next types` / `built-bin smoke`）。**先拍口径再动手**：根 `scripts/*.spec.ts` 因不在 `tests/` 下而漏进语料（现有 ignore 只有 `**/tests/**`）、55 行类型声明重复是否算债、组件样板是否该抽。（**frontier — 含决策点**）
