...
## Post-ship subagent review (2026-09-03)

T12 ship 后跑 subagent 独立 code review(static)+ runtime 验证(gates + adversarial 并发探针),镜像 T9 → T12 的 review 流程:

- **Subagent A(static code review)**:verdict **shippable**。确认 prior-review 全 18 项(HIGH 1-4 / MEDIUM 5-9 / LOW 11-15 / NIT 16-19)皆 fixed(NIT 18 intentional not-applied + documented;LOW 11/12/14/19 deferred-to-README)。发现 **1 LOW latent bug**:sync-throwing fetcher 泄漏 inFlight slot——IIFE 的 `finally` 在 `inFlight.set` 之前跑,故 sync-throw 时 `inFlight.get(key)` 仍 undefined → 不删 → entry 以已-rejected 的 pending + 已执行的 finally 存入 → 后续 `get` 返 stale rejected promise 而非 refetch。latent(production `service.fetch` 是 `async`,throw 走 Promise reject 不 sync-throw;但 `ResultFetcher` 契约 TS-允许 sync-throw `() => { throw }` ⊆ `Promise<...>`)。**修**:`inFlight.set(key, entry)` 移到 IIFE 调用之前(1 行 reorder),finally(sync-throw 或 async-reject)皆见 entry 在 map → 正确 delete。另 **2 NIT 非 bug**:schemastery `Config` `.default(DEFAULT…)` + `apply` `config.x ?? DEFAULT…` 双默认(harmless overlap,`ui-semantic-layer` precedent 同);`Config` parse 路径(cordis.yml)未测(merge 逻辑已测,schemastery 侧靠 precedent + cordis)。
- **Subagent B(runtime verification)**:gate 全独立复绿(`tsc -b` exit 0 / 29 vitest / bundle 84kB / 4 doc-sync 门对 `client/result-cache` clean;残留 `data/result-cache` 失败是**他包**,pre-existing,out-of-scope);**7 个 adversarial 并发探针全 pass**(invalidateAll-during-in-flight、transport-throw-then-get、two-keys-concurrent、abort-during-in-flight、encodeURIComponent key collision、multi-invalidate-during-one-flight、single-flight confirm);no bugs found via probes。
- **Fix landed(同 session,即时闭)**:`fix(client/result-cache): release in-flight slot on sync-throwing fetcher`——`cache.ts` `inFlight.set` reorder + `tests/result-cache.client.spec.ts` 加 `syncThrowing` helper + regression test(30 tests green:2 gets → 2 fetches,无 stale-reject 泄漏)。属 T12 自身硬化(review 找的 latent bug 即时闭),**非新 ticket**(latent + 1 行修 + TDD-clean,即时闭优于 graduate 一张 linger ticket)。
