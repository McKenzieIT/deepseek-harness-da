...
- [T12: Harden T9 result-cache per code review](tickets/T12-harden-result-cache-per-review.md) — T9 code-review 修补（无新决策）:HIGH（in-flight invalidate epoch guard 防 late `lru.set` 存旧快照 + single-flight `Map<key,InFlight>` 合并并发 get + `ResultFetchError` value-export 供 `instanceof` + transport-throw 折叠 `code:'transport'`，fold 在 `cache.get` 单点可测）+MEDIUM（schemastery `Config` 纠正 Agent Note 错 claim + drop no-op `updateAgeOnGet` + `JSON.stringify` 单次传 `{size}` + `apply.client.spec` 测 `connection/reset`→invalidateAll + `Config` merge + `invalidateSession`/`Scope` owner 列 deferred）+LOW/NIT（`encodeURIComponent` key 防撞 + `scopeId` 返 `SessionId`+op-label+`.get('results')` message + dead `import type{}` + 冗余 `ISessions` cast）;`ResultFetchError.code:string` widen 保留（transport arm 需非-RpcErrorCode code,NIT 18 不应用）;29 tests + tsc + bundle + test:gui + 4 doc-sync 门对**本包**全绿;runtime FakeApiClient residual（T8 同源）graduate [T13](tickets/T13-runtime-fakeapiclient-results-arm.md)、result-cache `.mock` tsc 错（T9 测试 cast）本票修。**post-ship subagent review**：1 LOW latent（sync-throw inFlight slot leak）即时修（同 session commit，非新 ticket），2 NIT 非 bug（schemastery 双默认 / Config parse 未测），7 runtime 探针全 pass。resolves → [T10](tickets/T10-consumer-fetchResult-wiring.md) unblocked
- [T13: Fix runtime FakeApiClient `results` arm (T8 residual, runtime half)](tickets/T13-runtime-fakeapiclient-results-arm.md) — runtime `FakeApiClient.results` arm（返 `result-not-found`）由并发 session 提交于 `e9bc392f60`(broad "GA-AUDIT1-followup" sweep,非 clean T13 pathspec commit);本 session 验证收口:聚合 `tsc -b tsconfig.client.json` 复绿(exit 0,WIP 既入 HEAD 故绿无条件)+ 定向 runtime/ui-conversation 811 测试绿(0 失败)+ 全 `test:gui` 4243 passed/5 failed(5 失败皆 `ui-semantic-layer` 并发 WIP `useStore is not a function`,与 T13 无关)。T8 residual runtime 半(T11=connection 半/T12=cache 硬化/T13=runtime 半)全清,R5 数据线 T8 residual 闭环。
