...
19g. **`test:snapshot`**: upstream added `scopeId` to `ToolExecutionInput`, so the recorded `cordis-inspect-jsdoc` session no longer matched what the live inspect tool reports. `DSH_SNAPSHOT=refresh` replayed the scenario keylessly and rewrote exactly one recorded line; the whole corpus replays green locally (4 files). The same job still fails on CI for a **separate, upstream-owned** reason: `initialize timed out after 10000ms waiting for dsh profile "sdk"`, a different scenario each run. The `sdk` profile, its plugin roster, the SDK client's 10s handshake default, and that snapshot file are all upstream's, and the fork mounts no da plugin there; PR #167 on the pre-merge base does not hit it, so upstream's 666 commits made that profile's cold boot exceed upstream's own budget on CI hardware. Treated as upstream debt: not fixed, not retried, not weakened — evidence in [UM18](../wayfinder/data-agent/tickets/phase-upstream-merge/UM18-post-0d1f50007f-residual-red-gates.md).
...
| PR #168 CI | third run: 20 pass / 3 fail — `config catalog` (two jobs, the ticketed bilingual backlog) and the upstream-owned sdk handshake budget; every merge-caused failure is fixed |
