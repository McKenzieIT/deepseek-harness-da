...
packages/    @deepseek-ai/dsh-<pkg> workspaces at packages/<group>/<pkg>/
  ...
  subprocess/  subprocess + local process-tree provider
  terminal/         persistent sessions
  fs/          filesystem + policy
  lsp/         language server
  skill/       skill provider registry + local impl + catalog/loader tool
  web/         web: Service Definition + search/fetch providers + tool Consumer
  compaction/     compaction + basic provider
  ...
  credentials/ credential-reference + env/.env provider
...
native/      @deepseek-ai/node-addon-landlock-run source (see native/README.md)
...
pnpm run test:snapshot  # keyless ACP/headless replay vs expected; filter: -t <name>
...
When `gh`, `pnpm`, build, test, or generator commands fail because the sandbox blocks credentials, network, IPC, file watching, or nested `sandbox-exec`, retry unchanged with the narrowest host escalation before diagnosing auth or project failure. Require sandbox evidence; never bypass genuine test failures.
...
- Match evidence to the surface: focused tests for behavior, snapshots for model/user output, `doc-sync` for docs, build/hygiene smokes for published paths, real-API e2e for provider behavior.
- Never default to the full suite or repeat a passing check. CI owns exhaustive coverage and the platform matrix; rehearse all locally only by explicit request, CI diagnosis, or an irreducibly repo-wide change.
...
- **Runtime invariants assert owned relationships.** Check authoritative event streams or mutable data, not service/method presence, plugin metadata, or fixed examples. Without a plausible relationship, an explained empty companion is correct ([package invariant rules](packages/AGENTS.md)).
...
- **Testing policy** — [docs/testing.md](docs/testing.md). Every non-trivial model- or product-user-visible behavior change adds/updates a keyless snapshot via a real runnable example in the same PR; package tests, e2e-only assertions, and mock-only fixtures do not substitute. Fixtures must replay on macOS/Linux; fix fixtures, not normalizers.
...
See [docs/defensive-patterns.md](docs/defensive-patterns.md) for lifecycle, concurrency, subprocess, or teardown work.

## Type safety and documentation

Everything compiles under `strict: true` with `noImplicitAny`; every remaining `any` explains why narrowing is infeasible. Every module and export has concise JSDoc; function-like exports include `@param`/`@returns` (enforced by `verify-export-jsdoc`). Heritage-declared members, plugin-protocol slots, and constructors keep docs at the declaring Service Definition, protocol, or class.

Comments and docs state complete contracts and context, not reasoning transcripts. Use direct, concrete terms; no metaphors. Before writing `contract`, `boundary`, or `shape`, ask whether a more exact term names the subject (e.g. `response fields`, `JSON validation`, `ESM exports`). Keep `contract` for preconditions, postconditions, invariants, and compatibility obligations; keep a literal process, wire, security, transaction, or lifecycle boundary. Do not narrate control flow/tests, preserve review history, or restate code. Keep behavior, failure, timing, ownership, and safe-use facts; link the rationale. Use [dsh-prose-standard](.agents/skills/dsh-prose-standard/SKILL.md). Wire mechanically checkable invariants into an executed top-level gate; prove each changed acceptance path rejects an invalid case. Use narrow exceptions instead of disabling a rule globally.

Docs accompany every code change: update affected README and JSDoc together. Routine bilingual work follows [docs/AGENTS.md](docs/AGENTS.md); only explicit user invocation runs `dsh-translate-docs`. Current-state prose, one physical line per paragraph, one home per fact, and word budgets live there.

## Editing these instructions

`CLAUDE.md` symlinks `AGENTS.md` at root, `packages/`, and `examples/`; edit the real file. Keep rules self-contained while linking high-level docs. Condense when clarity survives; raise a `verify-doc-budgets` ceiling when content genuinely needs more space.

## Vendoring policy

`vendor/` packages are pinned source copies (manifest + upstream SHAs in [vendor/README.md](vendor/README.md)). Update via the sync procedure there; re-apply or retire logged local modifications; rerun `pnpm run test && pnpm run build`.
