...
- **Decision-informing experiments are audited.** A probe, recall/ambiguity measurement, or A/B run whose results inform a decision persists its results — setup (corpus/cases/scope/config + what is varied), the measured numbers (verbatim, not paraphrased), the verdict, a fidelity caveat (any port-vs-shipped divergence in tokenizer/idf/floor/weights), and a pointer to the deciding ticket — to the effort's experiment-audit log (`wayfinder/semantic-layer/research/experiment-audit-log.md` for the semantic-layer effort; an equivalent log per other efforts), not only in ticket prose or throwaway script output. The probe script is committed alongside; the log is the durable evidence future sessions cite when revisiting or re-measuring the decision.
- **Every eval run MUST be recorded.** LLM outputs are non-deterministic and non-reproducible; an unrecorded eval run is a lost experiment. After each eval run: (1) use `packages/eval/eval-cli/bin/compare.ts` to diff against the previous baseline run, (2) append a standard entry (Setup → Data verbatim → Verdict → Ticket Pointer) to the experiment-audit-log. Template in `packages/eval/eval-cli/README.md`. This applies to full batch runs, not single-case debug probes — unless the result influences a decision.
