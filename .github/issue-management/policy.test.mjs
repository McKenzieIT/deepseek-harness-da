...
test('keeps trusted preflight before token minting and policy gated only on the owner', () => {
  ...
  // The owner gate is the job's only condition: forks carry no DSH issue App, so
  // the check reports skipped there while staying unconditional for the owner.
  const beforeSteps = job.slice(0, job.indexOf('    steps:'))
  assert.ok(beforeSteps.includes("    if: ${{ github.repository_owner == 'deepseek-ai' }}"))
  assert.equal(beforeSteps.match(/^ {4}if:/gm).length, 1)
