...
| [data-agent.md](data-agent.zh.md) | data-agent 叠加层的五个 Cordis 服务：ctx.audit（只追加的审计存储）、ctx.embedder（嵌入/重排 seam）、ctx.identity（调用方身份）、ctx.nl2sql（自然语言转 SQL 引擎）、ctx.schema（语义层 discover/describe/sample） |
