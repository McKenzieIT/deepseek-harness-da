...
| [data-agent.md](data-agent.md) | data-agent 叠加层的五个 Cordis 服务：ctx.audit（只追加审计存储）、ctx.embedder（embedding/rerank seam）、ctx.identity（调用方身份）、ctx.nl2sql（NL 转 SQL 引擎）与 ctx.schema（语义层 discover/describe/sample） |
