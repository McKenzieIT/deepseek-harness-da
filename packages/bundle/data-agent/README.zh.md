...
已挂载的 eval service 会显式标识 `query-maxcompute` / maxc-sidecar 组合，采用与 provider `toolCallTimeoutMs` 一致的 300 秒墙钟截止时间，在 profile 加载时按 UTC 日期解析 `today`，并记录 `by-name` 列语义与 200 行 artifact 上限。缺失必要基础设施或执行器归因时会在评分前明确失败，不会计作模型失败。
