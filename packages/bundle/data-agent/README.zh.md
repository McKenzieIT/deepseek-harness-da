...
评测运行由外部 `dsh-eval` host 承担。默认产品 profile 不挂载 `eval-runner-service`：发布包不包含 benchmark cases，普通 data-agent session 也不应常驻评测基础设施。

## Model Experience

间接：经其禁用与挂载的行——本 bundle 自身不产 model-visible 文本。它挂载 `llm-dashscope`（P2）作为 profile 的直连 LLM，且已发布的 data 能力插件（P4-P11：`query-maxcompute`、`semantic-layer`、`nl2sql-engine`、`schema-gateway`、`evidence-query`、`audit`、`admin`、`result-cache-memory`、`preset-autojoin`、goal/eval context 对、`code-runtime-data-python`）各自向组合树贡献其 model-visible schema、prompt 与 tool 定义。部署选择行（`embedder`、`retrieval`、`subagent-qoder`）在挂载 provider 前不挂任何东西。
