...
data-agent bundle：叠在 [`dsh-base`](../base/README.md) 之上的 additive patch 层，服务 `data-agent` profile。[`cordis.patch.yml`](cordis.patch.yml) 禁用 code-agent 面——`tool-str-replace-editor` 与 `tool-ralph` 两行，加 `tools.mode: native` 关 Code Mode——disable-only、不删，上游 `dsh-base` 重排也不会让它重现。它预留一块注释占位的 `insert` 块，待 data 能力插件包就绪后挂载（P4-P11：query/retrieval/embedder/semantic-layer/audit/admin，加 phase-1 的 `llm-dashscope`/`subagent-qoder`）。当前不 insert 任何东西：指向尚未发布包的 bare specifier 会炸 `pnpm install` 与 `verify-cordis-config`，故每行 data 插件在包就绪前保持注释。data-agent 的 persona 不在此设——归四阶段 preset（P7）。`tool-bash` 与 `code-runtime` 是 data agent 自用的执行后端（map Q9）；此处保持启用，业务用户的门禁在 P10 内网工具层做，不在本 bundle。
...
间接：经其禁用与待挂的行——本 bundle 自身不产 model-visible 文本，P4-P11 填充预留块前不挂任何插件。
...
- **当前不挂任何 data 插件**——每行 data 能力都是注释占位；待对应 ticket（P4-P11、P2、P3）发包后再取消注释并填 `name:`。
