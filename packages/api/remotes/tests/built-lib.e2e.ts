    ...
    const script = `
      ...
      const instantiate = id => {
        ...
        return handoff.factory(specifier => {
          // Fork's staticModules table (single-source with platform.ts/seed.ts)
          // answers every platform external the api-remotes bundle emits —
          // cordis, zod, and the fork's schema-gateway/evidence-query namespaces
          // — so it is consulted first. Upstream's direct cordis/zod checks
          // remain as a fallback so a table gap never breaks the chain.
