...
候选 Agent 运行前，每个通过结构解析的 case 都会检查评分内容。有 executor 时会执行已解析的 reference SQL：环境失败归 `infra_failure`，reference SQL 无效或结果与已声明 expected 不一致归 `case_defect`，两者都不进入模型错误分母。每个 `CaseVerdict` 都携带对应 preflight evidence。
