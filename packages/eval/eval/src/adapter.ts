 ...
 *
 * `extractReply` takes `finalResponse` directly as `reply` (the TS SDK gives
 * it; rbi concatenates TEXT events) and parses the agent's generated SQL out of
 * the `tool/call` event (the agent's query tool call). The eval then RE-RUNS
 * that SQL via the injected `CaseSqlExecutor` for a deterministic actual (G2
 * "跑 da 自己的 ODPS"), not the agent's trace `tool/result`.
 *
 ...
 * steering / queued work could contribute before idle. Research established
 * this does not bite for da scripted multi-turn (single prompt, no steering,
 * no queued work). The adapter **asserts** the mitigation: `validateRunResult`
 * checks the interval has exactly ONE `assistant/message`; a derailing interval
 * (≥2) raises `ProtocolError`. S8 exercises this.
 *
...
/** Raised when the H1 mitigation trips: a run interval did not carry exactly one `assistant/message`. */
...
/**
 * Assert exactly ONE `assistant/message` per run interval (H1 mitigation).
 * @param runResult - one run's `RunResult`.
 * @throws {ProtocolError} when the count is not 1.
 */
export function validateRunResult(runResult: RunResultView): void {
  ...
  if (count !== 1) {
    throw new ProtocolError(
      ...
      + '(finalResponse is "last assistant/message, not causally assigned to prompt" — a derailing '
      + 'interval; research Claim H1 says scripted single-prompt mode does not trigger this, so '
      + 'a count != 1 is a real fault)',
    )
  }
}
 ...
 * `reply = finalResponse`; `generatedSql` = the first SQL found in a `tool/call`
 * event. An empty reply does not raise (rbi: "the agent said nothing" is
 * gradeable — the session derails — not an infrastructure error).
 * @param runResult - one run's `RunResult`.
...
export function extractReply(runResult: RunResultView): AgentTurnReply {
  ...
  for (const e of runResult.events) {
    ...
    if (generatedSql !== null) continue
    const sql = firstSql(e)
    if (sql !== null) generatedSql = sql
  }
 ...
 */
function firstSql(e: RunResultEvent): string | null {
  const data = e.data
