...
>
> `rebaseline-passk-168-clean` (the row marked CURRENT) **does not exist as a file anywhere**: checked `eval-results/` in all 8 worktrees plus a repo-wide `find` — zero hits. Cause: `eval-results/*.json` is gitignored (`.gitignore:61`), so run artifacts live only on the machine that produced them and vanish with the worktree.
>
> Consequences, concretely:
> - **`node --import tsx/esm packages/eval/eval-cli/bin/compare.ts rebaseline-passk-168-clean <new>` cannot run.** Every "compare against the baseline" instruction in this README (including the reproduce line below) is currently unexecutable.
> - CLAUDE.md mandates recording every eval run, and CL-22 mandates ≥3-run medians — both assume the artifacts persist. They do not.
> - The recorded NUMBERS are still trustworthy (they are in this table and in experiment-audit-log.md); it is per-case comparison and re-scoring that are lost.
>
> A 2026-09-06 CL-20 session hit this directly: it was told to compare against `rebaseline-passk-168-clean` and could not, so it compared against two same-protocol k=1 runs that did still exist. Decision pending in [CL-29](../../../wayfinder/semantic-layer/tickets/CL29-eval-artifact-persistence.md).
...
| pass@3 pass^k, **real-exec** post-(a)+(d) eventdef-prefetch (RBI 10000251, 39 EXEC) | qwen3.7-max | `eventdef-realexec` | 2026-09-06 | **2/39 = 5.1%** as-shipped · **5/39 = 12.8% re-anchored**; **null-SQL 23→13 /117 (−43%)**; `FROM <数据视图>` placeholder attempts **3→0**; attempts using `ieu_ods.ods_10000251_all_view` **0→18** (all 6 detected event cases; **0 DWS cases**). ⚠ **The as-shipped rate is not interpretable at this n**: the passing set moved {036,037,039} → {041,046} — zero overlap, all DWS — so `passKVerdict=every` on n=39 makes it a lottery among DWS cases. ⚠ **16/18 event cases' expected values are stale** (the raw event ODS view's historical partition is not a frozen anchor; DWS 13/13 still exact) — re-scored against each case's own reference SQL run live, event cases go **0/18 → 3/18**, with 119/125/126 hitting the live value exactly on all 3 attempts. See GA-EVAL-CASESET-EVENT-ANCHOR + `dev/reanchored-score.mjs` |
| pass@3 pass^k, **judge-only** post-(a)+(d) eventdef-prefetch (RBI 10000251, 39 EXEC) | qwen3.7-max | `eventdef-judgeonly-v2` | 2026-09-06 | **24/39 = 61.5%** judge-only (was 53.8% post-(d)); **null-SQL 21→13 /117 (−38%)**; non-SQL emissions **0%** maintained; attempts using `ieu_ods.ods_10000251_all_view` **0→18**. Gains land on the cases (a) targets: +9 vs (d), 7 of them event cases (057/119/125/126/128/136/137). The 6 regressions were checked per-case: 5 are DWS cases where detection never fired (`eventView=false` on all 3 attempts, prompt byte-identical to (d)) = `passKVerdict=every` amplifying attempt-level sampling; 1 (135) is judge source-ambiguity on 1 of 3 attempts. ⚠ A first run (`eventdef-judgeonly`, 41.0%) measured the **judge** blind to the event view — see the GA-EVAL-EVENTDEF-PREFETCH entry |
| pass@3 pass^k, **real-exec** post-prompt-fix (RBI 10000251, 39 EXEC) | qwen3.7-max | `rebaseline-real-exec-rbi-10000251-postpromptfix` | 2026-09-05 | **3/39 = 7.7%** real-exec; non-SQL tool-call emissions **0% (was 16.2%)** ✓ criterion #1 met; criterion #2 NOT met (dropped 12.8→7.7 — hypothesis refuted for real-exec; the real-exec bottleneck is SQL-correctness/wrong-values, not non-SQL emissions — engine responder doesn't pre-fetch `eventDef`) |
| pass@3 pass^k, **judge-only** post-prompt-fix (RBI 10000251, 39 EXEC) | qwen3.7-max | `rebaseline-judge-only-rbi-10000251-postpromptfix` | 2026-09-05 | **22/39 = 56.4%** judge-only; non-SQL **0% (was 22.2%)** ✓; criterion #2 met for judge-only (rose 48.7→56.4 — judge passes more semantically-plausible-but-wrong-value SQL; the judge-leniency pattern; both within n=39 noise MDE~20pp) |
