---
description: "TODO: translate: Model-facing query_data tool: executes SQL via ctx.query and returns rows for the data agent's EXECUTION phase"
...
TODO: translate: Model-facing query_data tool: executes SQL via ctx.query and returns rows for the data agent's EXECUTION phase

## 目录

- [状态：maxc-backed EXECUTION (P4c)](#status-maxc-backed-execution-p4c)
...
Model-facing `query_data` tool：data agent `EXECUTION` 阶段的**经 `ctx.query` 真实执行 SQL**。agent 调用它（传 SQL + per-game scope）经 query engine 跑该 SQL 并取回 rows——agent 跑它自己写的 SQL，而非 eval harness 重跑 canned 语句。
...
核心 EXECUTION 流（`executeQuery` / `projectOutcome` / `pollToSettlement`）以 pure function 导出，使 3-state 处理可对 stub `QueryEngine` 测试；P4c(c) smoke 对真 provider（maxc-backed sidecar→真 engine rows）调用它，证 tool 路径——非直 sidecar 调用。注册基于 effect（dispose plugin fiber 即注销 tool）；schema 自动流入 system-prompt 装配。见 [`docs/cookbook/adding-a-tool.zh.md`](../../../docs/cookbook/adding-a-tool.zh.md)。
...
query engine **非** config 字段：它是 `ctx.query`（data-agent bundle 挂载的 query provider）。`scope_id` 是 per-game access-isolation scope（信任边界）；生产加固从 `ctx.identity` 取，而非 model 传入。
...
未发布运行时 invariant companion，因为 `@deepseek-ai/dsh-query-tool` 不拥有可能与其运行时状态独立发生分歧的可观测关系。
...
- **`mode` 是 prototype-only knob** — `QueryRequest.mode`（`fast`/`slow`/`blocking`/`fail`）是 stand-in-sidecar 机制；maxc provider 从真 engine 执行派生 pending vs completed，不携 mode，故本 tool 不向 model 暴露它。
