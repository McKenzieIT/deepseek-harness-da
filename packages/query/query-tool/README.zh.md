...
核心 EXECUTION 流（`executeQuery` / `projectOutcome` / `pollToSettlement`）以 pure function 导出，使 3-state 处理可对 stub `QueryEngine` 测试；P4c(c) smoke 对真 provider（maxc-backed sidecar→真 engine rows）调用它，证 tool 路径——非直 sidecar 调用。注册基于 effect（dispose plugin fiber 即注销 tool）；schema 自动流入 system-prompt 装配。见 [`docs/cookbook/adding-a-tool.md`](../../../docs/cookbook/adding-a-tool.md)。
