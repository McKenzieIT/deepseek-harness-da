...
export interface ToolExecutionInput {
   ...
   * `undefined` — the dormant default, since no caller sets
   * `AgentOptions.scopeId` yet — leaves tools to fall back to the active scope,
   * preserving the pre-Phase-4 ({@link SemanticLayerService}/retrieval
   * `scopeId?` β) behavior. Phase 5 call sites begin resolving a tenant→scope
   * and supplying it here.
   */
