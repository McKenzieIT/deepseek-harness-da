...
import type { CredentialAddress, CredentialInfo, CredentialKey, CredentialRecord, CredentialRef } from './types.ts'

export type {
  ApiKeyRecord, CredentialAddress, CredentialInfo, CredentialKey, CredentialRecord, CredentialRef, GrantRecord,
} from './types.ts'
...
export abstract class CredentialProvider extends Service {
  ...
  protected notifyUpdated(ref: CredentialRef, address?: CredentialAddress): void {
    this.fanOut('credentials/reference-updated', ref, address)
  }
  ...
  private fanOut(
    ...
    const args = address === undefined ? [event, subject] : [event, subject, address]
    for (const listener of this.ctx.events.dispatch('emit', args) as Array<(...listenerArgs: unknown[]) => unknown>) {
      try {
        const returned = address === undefined ? listener(subject) : listener(subject, address)
