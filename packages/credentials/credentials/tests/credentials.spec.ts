...
import { credentialRef, isCredentialKeySegment, scopeId, userId } from '../src/index.ts'
...
describe('per-user/scope addressing through the memory provider', () => {
  it('isolates a per-user value from the global slot and from other users', async () => {
    ...
    ctx.on('credentials/reference-updated', (ref, address) => events.push({ ref, ...(address !== undefined ? { address } : {}) }))
