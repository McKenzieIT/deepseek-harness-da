...
- **At rest**: ✓. The keychain database is encrypted on disk, so `bash` `cat`/`grep` of the file yields ciphertext, not the PAT — the bar [`credentials-local`](../credentials-local/README.md#use-this-package) reserves for an OS-keychain provider. An independent keychain with a short auto-lock and lock-on-sleep, re-locked on the harness's teardown, narrows the window further: while locked, no process can read an item without the unlock password.
