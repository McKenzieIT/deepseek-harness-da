...
Per-item Touch-ID ACL (reads restricted to the harness binary, excluding `bash`/
`terminal`) is therefore **deferred to ticket P12c** — a native Security-framework
binding plus harness code-signing. The locked keychain is an at-rest and when-locked
enhancement, not a substitute: it narrows the runtime-exfil window to the unlocked
period, it does not close it.
...
- **Runtime-exfil ACL is deferred (P12c)** — see [Security boundary](#security-boundary): per-item Touch-ID ACL + harness code-signing is a native-binding + distribution project, not achievable in this `security`-CLI-only, additive package.
