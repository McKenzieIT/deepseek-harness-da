...
- **盘上（at-rest）**：✓。钥匙串数据库盘上加密，`bash` `cat`/`grep` 文件只得密文，非 PAT——即 [`credentials-local`](../credentials-local/README.zh.md#use-this-package) 为 OS 钥匙串提供方保留的那条红线。独立钥匙串 + 短 auto-lock + lock-on-sleep，并在 harness teardown 时再上锁，进一步收窄：锁定时，无密码任何进程都读不到条目。
