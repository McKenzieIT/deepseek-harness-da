---
description: "TODO: translate: Model-facing load_event_definition tool: load a validated event (埋点) definition from the semantic-layer substrate for the data agent's UNDERSTANDING/GENERATION phase"
...
TODO: translate: Model-facing load_event_definition tool: load a validated event (埋点) definition from the semantic-layer substrate for the data agent's UNDERSTANDING/GENERATION phase
...
注册基于 effect（dispose plugin fiber 即注销工具）；schema 自动汇入 system-prompt 装配。`execute` 返回单一规范 JSON 值（`{ found, event?, message? }`）；`output.render` 投影为面向模型的文本。见 [`docs/cookbook/adding-a-tool.zh.md`](../../../docs/cookbook/adding-a-tool.zh.md)。
...
### `load_event_definition` 工具调用

#### 模型所见

`load_event_definition` 工具 schema（name、description、`event_name` 参数、`found`/`event`/`message` 输出形态）在插件挂载后自动汇入 system-prompt 装配，模型在 `UNDERSTANDING`/`GENERATION` 阶段白名单中与之一起发现。模型调用时，`execute` 返回单一规范 `{ found, event?, message? }` JSON 值，`output.render` 投影为面向模型的文本：命中时为多行定义块（`event: <name>`、description、`event_filter`、`params_fields:` 列表、`metrics:`、`disambiguation:`、`external_refs:`），否则为单行 not-found / not-mounted / invalid-name 消息。

#### Token 影响

工具结果中渲染的定义文本是本工具唯一的逐调用 token 成本；`load_event_definition` schema 骑在 system prompt 而非 turn 负载。文本承载 SQL 落地相关字段（params_fields、指标、消歧、外部维度引用）；工作流状态字段（confirmation / coverage）在文本中省略，但 JSON 值承载完整已校验定义。成本随事件的 params_fields/指标数伸缩。
...
- **live-engine provider deferred（P6b Q3）** —— `ctx.schema.discover`/`describe`/`sample`（live-engine schema）在真实 query provider 挂载前抛 "no provider"（P6b follow-up）。`load_event_definition` 仅读 substrate 定义，故不阻塞。
