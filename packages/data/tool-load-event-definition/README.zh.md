...
注册基于 effect（dispose plugin fiber 即注销工具）；schema 自动汇入 system-prompt 装配。`execute` 返回单一规范 JSON 值（`{ found, event?, message? }`）；`output.render` 投影为面向模型的文本。见 [`docs/cookbook/adding-a-tool.md`](../../../docs/cookbook/adding-a-tool.md)。
