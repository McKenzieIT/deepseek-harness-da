...
- **Live-engine provider deferred (P6b Q3)** — `ctx.schema.discover`/`describe`/`sample` (live-engine schema) throw "no provider" until a real query provider mounts (P6b follow-up). `load_event_definition` only reads the substrate definitions, so it is unblocked regardless.
