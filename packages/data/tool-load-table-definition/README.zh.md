...
`ctx.schema` bundle service 行（`packages/bundle/data-agent/cordis.patch.yml` 的 `semantic-layer`）为 **deferred follow-up**：因并发会话正改 bundle patch + lockfile（避撞车），本 ship 暂缓。解注释该行 + 加 `dsh-semantic-layer` bundle dep（需 `pnpm install` 保 lockfile 同步）即挂载 `ctx.schema`、接通本工具到真 substrate；在此之前返回 "not mounted"。默认空 `semanticRoot` 下，已挂载 substrate 不扫任何 `tables/` 目录，返回 `null`（not-found，不崩）；真实 substrate 目录在 profile/运行时层配置。
