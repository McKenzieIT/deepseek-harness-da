...
- query_data(sql): 执行 SQL（仅 SELECT，必带分区过滤）；内置 CostGuard+探索预算（MAX_SQL_PER_TURN=${MAX_SQL_PER_TURN}）；返 3-state（done+result_id / running+instance_id / failed+error+failureKind）（P4 ctx.query.execute）
