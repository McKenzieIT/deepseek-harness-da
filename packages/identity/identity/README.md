...
`ctx.identity.current()` resolves the caller a request acts for — which business user (Qoder PAT authn), tenant, and scope (data isolation) — so per-user credentials and per-user audit attribute to the right principal.
...
None, as the identity seam resolves the caller a request acts for and never enters a model request; it is model-hidden metadata for auth, audit, and data isolation.

#### KV Cache effect

No effect; identity is caller metadata read by the keychain and audit services, not by any model request.
