...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  },
  {
    ...
    occupants: [
      ...
      'client-ui-semantic-layer SemanticLayerGoalDock id \'semantic-layer-evidence\'',
    ],
    replaceRisk: 'none',
  ...
  },
  {
    ...
    kind: 'list',
    scope: 'session',
    summary: 'A coexisting occupant of the right details column — a session-scoped list seat rendered beside `details`/DetailsPanel, so a feature panel (the semantic layer\'s EvidenceSidebar) shares the column without replacing the tool-call details viewer.',
    doc: 'A coexisting occupant of the right details column — a session-scoped\nlist seat rendered beside `details`/DetailsPanel, so a feature panel\n(the semantic layer\'s EvidenceSidebar) shares the column without\nreplacing the tool-call details viewer. An empty list renders nothing,\nleaving DetailsPanel intact.',
    registerOptions: [
      {
        name: 'id',
        requirement: 'required',
    ...
    declaredBy: 'an entry in \'root\' (client-ui-layout), so it exists while that entry is mounted',
    occupants: [
      'client-ui-semantic-layer SemanticLayerEvidence id \'semantic-layer-evidence\'',
      'client-ui-semantic-layer SemanticLayerSchemaExplorer id \'semantic-layer-schema-explorer\'',
    ],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'details.aux\', () => ctx.slots.register(\n      { name: \'details.aux\', id: \'my-entry\', order: 100, label: \'My entry\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
  ...
  },
  {
    ...
    declaredBy: 'an entry in \'root\' (client-ui-layout), so it exists while that entry is mounted',
    occupants: [
      'client-ui-context-layer ContextLayerOverlay id \'context-layer-fullscreen\'',
    ],
    replaceRisk: 'none',
  ...
  },
  {
    ...
    declaredBy: 'an entry in \'sidebar\' (client-ui-sidebar), so it exists while that entry is mounted',
    occupants: [
      'client-ui-semantic-layer SemanticLayerShell id \'semantic-layer\'',
  ...
  },
  {
    ...
    keyDomain: 'open: any string the owner dispatches (no compile-time key set), already taken: ask_user_question, bash, cordis_define, cordis_run, cordis_stop, cordis_undefine, discover_relations, edit, get_coverage, get_definition, glob, grep, present_decomposition, present_table, read, read_image, search_schema, skill, suggest_followups, todo_write, trigger_eval, web_fetch, web_search, write',
    keyDomain: 'open: any string the owner dispatches (no compile-time key set), already taken: ask_user_question, bash, cordis_define, cordis_run, cordis_stop, cordis_undefine, edit, glob, grep, present, read, read_image, skill, todo_write, web_fetch, web_search, write',    hookContext: '',
    slotInject: '',
    declaredBy: 'an entry in \'conversation.chat.node\' (client-ui-tool), so it exists while that entry is mounted',
    occupants: [
      'client-ui-present-decomposition DecompositionCard key \'present_decomposition\'',
      'client-ui-present-table TableCard key \'present_table\'',
      'client-ui-semantic-layer SearchSchemaRow key \'search_schema\'',
      'client-ui-semantic-layer GetDefinitionRow key \'get_definition\'',
      'client-ui-semantic-layer GetCoverageRow key \'get_coverage\'',
      'client-ui-semantic-layer DiscoverRelationsRow key \'discover_relations\'',
      'client-ui-semantic-layer TriggerEvalRow key \'trigger_eval\'',
      'client-ui-deliverables PresentRow key \'present\'',      'client-ui-skill SkillRow key \'skill\'',
      'client-ui-suggest-followups FollowupChips key \'suggest_followups\'',
