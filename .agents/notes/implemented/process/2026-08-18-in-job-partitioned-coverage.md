...
The Python runtime's multi-million-element memory-stress suite also runs uninstrumented. Its assertions exercise child-process `RLIMIT_AS` and O(depth) auxiliary allocation; ordinary runtime tests already cover the host TypeScript, while V8 instrumentation adds host frame-processing cost without strengthening the Python memory proof.
