...
[`upstream-monitor.yml`](../../../../.github/workflows/upstream-monitor.yml) runs both commands weekly at 21:17 UTC Monday (05:17 Asia/Shanghai Tuesday) and on manual dispatch, with `contents: read`, one non-cancelling run per ref and event, and the report retained as an artifact on failing runs too. The failed run is the notification; no Issue is filed, because automatic Issue creation would first have to define write permissions, deduplication, update, and closure behavior.
