...
Python runtime 的百万级元素内存压力套件也在无插桩门禁中运行。其断言验证子进程 `RLIMIT_AS` 与 O(depth) 辅助分配；普通 runtime 测试已覆盖 host TypeScript，而 V8 插桩只会增加 host 端 frame 处理成本，不会增强 Python 内存证据。
