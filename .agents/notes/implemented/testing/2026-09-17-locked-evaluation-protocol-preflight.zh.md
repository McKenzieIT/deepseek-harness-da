...
[Phase-gate incremental-value experiment](../../../../wayfinder/task-orchestration-dag/tickets/G25a-phase-gate-incremental-value-experiment.md) 应用了该规则。修订后的真实执行切片包含十个 L2 案例和两个 L3 案例。因此 Stage 1 使用显式批准的 3×L2 + 2×L3 + 一个持续失败案例组合；若 manifest 无法提供这些命名案例，controlled runner 会在消耗任何 smoke 或决策 Attempt 前拒绝运行。
...
协议预检可能在实现已经就绪时仍要求人工修订。这个暂停换来可复现的 Comparison Plan，保留决策 Attempt，并阻止基础设施可用性或操作者替换进入因果估计。
