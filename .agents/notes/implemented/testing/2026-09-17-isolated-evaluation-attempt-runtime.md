...
Stage admission remains separate from model grading. Reference SQL runs before and after Stage 1 with an explicit `MAXC_CONFIG`; a changed digest or expected-value mismatch stops the experiment. Tool and Task parity, model-visible Task inclusion, readable query outcomes, scorer safety, and infrastructure-failure rates are evaluated from sealed Attempt evidence before Stage 2 can begin. This complements the [locked protocol preflight](2026-09-17-locked-evaluation-protocol-preflight.md), which rejects an internally inconsistent manifest before external work starts.
