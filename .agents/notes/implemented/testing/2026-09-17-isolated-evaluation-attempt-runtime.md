...
Raw Session events, query rows, runtime failures, grader prompts and responses, blinded review packets, reveal maps, and per-Attempt launchers stay under the ignored `eval-results/g25a/raw/` tree. Committable summaries retain only identities, digests, categorical grades, tool names, cost counters, reference-result digests, and raw locators. The runner seals each observation before invoking the tool-free grader, salts blind review ids independently from Attempt ids, and disposes every Agent handle before its root context; all teardown is awaited.
