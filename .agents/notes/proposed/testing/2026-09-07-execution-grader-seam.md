...
4. The injected port stays one function, `(sql) => Promise<ExecutionResult>`. Hosts hand over a capability, not a verdict; evaluation does not depend on `MaxComputeQueryEngine` and does not grade through the model-facing `query_data` rendering layer.
