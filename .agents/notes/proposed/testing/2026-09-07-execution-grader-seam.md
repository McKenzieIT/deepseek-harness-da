...
- **The recorded case source fields are discarded at the file boundary.** All 39 cases in `packages/eval/eval/cases/rbi-10000251-exec/` carry `expected.sql`, `meta.anchor_ds`, `meta.tier`, and `meta.provenance`; `EvalCaseSchema` declares none of them and zod strips unknown keys (`eval/src/eval_case.ts:39-44`, `:54-58`), so a `loadCase` on such a file returns `expected` keys `result_value,match_mode,answer,delivery_match` and no `meta`. The reference SQL and snapshot anchor a replayable grade needs are present on disk and unreachable from the eval path.
