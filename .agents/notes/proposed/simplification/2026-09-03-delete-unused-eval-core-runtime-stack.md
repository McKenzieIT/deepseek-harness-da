...
**第二条路已经走完**：
- `bestOfKVerdict` → `passKVerdict`（`packages/eval/eval-runner/src/runner.ts:378`， `every(attempts)` 全中才算过），随 GA-AUDIT1 落地。
- 已 re-baseline：当前基线 `rebaseline-passk-168-clean` = **61.9%** pass^k，`eval-cli/README.md` 已声明 "pass^k semantics is LIVE" 并按新语义重设 Quality Targets。
- `RunResult.config` 现持久化 `pass_k` + `verdict_semantics`，`compare.ts` 已加 协议守卫（跨 k 比较 exit 2），`scripts/run-eval.sh` 的 `--pass-k 1` 偏离已移除。
