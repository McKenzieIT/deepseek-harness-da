...
describe('CI workflow', () => {
  ...
  it('keeps split native Windows PR jobs with failover, plus a master-only standby', () => {
    ...
    for (const [jobName, job] of [['node-24-coverage', node24Coverage], ['windows-coverage', windowsCoverage]] as const) {
      ...
      expect(steps, `${jobName} must select the provisioned interpreter`).toContainEqual(expect.objectContaining({
        env: { DSH_TEST_PYTHON_PATH: '${{ steps.data-python.outputs.python-path }}' },
...
describe('Issue lifecycle workflow', () => {
  it('allocates lifecycle runners only for events that can change the board', () => {
    ...
    // Upstream also skips a metadata-only pull_request edit (body unchanged).
    expect(lifecycle.on).toHaveProperty('pull_request')
    expect(lifecycle.on).toHaveProperty('pull_request_review')
    expect(lifecycleJob.if).toContain("github.repository_owner == 'deepseek-ai'")
  ...
  it('mints Project credentials only after preflight and always revalidates current metadata', () => {
    ...
    // The job itself carries the fork's owner gate: without the DSH issue App the
    // check must skip on non-upstream owners rather than fail.
    expect(policyJob.if).toBe("${{ github.repository_owner == 'deepseek-ai' }}")
