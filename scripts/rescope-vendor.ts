...
const GENERIC_SKIPS: readonly GenericSkip[] = [
  ...
  { file: 'scripts/gen-architecture-graph.ts', upstream: ['cordis'] },
  // The fork's mirror codemod names the bare upstream token as its example of a
  // name that also serves as product data.
  { file: 'scripts/rescope-fork.ts', upstream: ['cordis'] },
...
export function isRescopeExcluded(file: string): boolean {
  ...
  // Decision tickets and research records quote event ids and code as they stood.
  if (file.startsWith('wayfinder/')) return true
