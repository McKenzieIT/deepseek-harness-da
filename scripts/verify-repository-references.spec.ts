...
describe('maintained repository reference policy', () => {
  ...
  it('excludes the records that cite commits as their evidence', () => {
    const commit = 'a'.repeat(40)
    const commits = new Set([commit])
    expect(findRepositoryReferences('wayfinder/data-agent/tickets/T1.md', commit, commits)).toEqual([])
    expect(findRepositoryReferences('upstream-sync.json', commit, commits)).toEqual([])
    expect(findRepositoryReferences('upstream-sync/upstream-merge-plan.md', commit, commits)).toEqual([])
    expect(findRepositoryReferences('docs/da-upstream-debt.md', commit, commits)).toEqual([])
    expect(findRepositoryReferences('docs/architecture.md', commit, commits))
      .toEqual([{ file: 'docs/architecture.md', line: 1, kind: 'commit-hash' }])
  })
})
