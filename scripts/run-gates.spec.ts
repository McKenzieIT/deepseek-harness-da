...
describe('gate graph validation', () => {
  ...
  it('keeps the hygiene aggregate aligned with the package script checks', () => {
    ...
    expect(ids).toEqual([
      ...
      'optional-dependency-imports', 'client-packages', 'client-ui-i18n', 'no-bare-dispatcher', 'gate-coverage', 'upstream-sync-record', 'cordis-config',
      'runtime-closure',
    ])
  ...
  it('schedules the longest documentation leaves before short checks', () => {
    ...
    expect(ids.slice(0, 10)).toEqual([
      ...
      'type-equivalence', 'cordis-catalog', 'cordis-inspect-catalog', 'workflow-guest', 'mermaid',
    ])
  })
