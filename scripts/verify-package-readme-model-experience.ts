...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/experimental/agent-team-web-profile': { kind: 'indirect', reason: 'The Web layer delegates model composition to the Host-side Agent Teams profile.' },  'packages/experimental/client-ui-agent-team': { kind: 'none', reason: 'The browser projection and task controls register no model-facing input.' },
  ...
  'packages/client/ui-present-table': { kind: 'indirect', reason: 'Pure browser-side table-presentation operation; model integration lives in @deepseek-ai/dsh-nl2sql-engine\'s LLM adapter.' },
  ...
  'packages/bundle/data-agent': { kind: 'indirect', reason: 'The bundle is a patch-list carrier; each inserted row\'s package owns its model-facing behavior.' },
  ...
  'packages/llm/llm-dashscope': { kind: 'none', reason: 'The native-protocol adapter forwards the already-assembled harness request unchanged, re-encoding it for the DashScope wire; it adds no model-bound text, schema, or message.' },
  ...
  'packages/credentials/credentials-keychain': { kind: 'indirect', reason: 'The macOS keychain provider stores credential values; the consuming adapter owns every model-facing use a value authorizes.' },
  'packages/credentials/credentials-keychain-host': { kind: 'indirect', reason: 'The mount face registers a model-agnostic credential provider; the consuming LLM adapters own every model-facing use a resolved value authorizes.' },
  ...
  'packages/identity/identity': { kind: 'none', reason: 'The identity seam resolves the caller a request acts for and never enters a model request; it is model-hidden metadata for auth, audit, and data isolation.' },
  'packages/eval/eval': { kind: 'none', reason: 'Model-agnostic eval test harness; the host injects the responder, executor, and judge collaborators, so the package registers nothing model-facing.' },
  'packages/retrieval/retrieval': { kind: 'indirect', reason: 'The retrieval seam delegates model-facing use to search_data_sources and the nl2sql schema-linking pipeline; a dedicated retrieve tool is deferred (D2c scope).' },
  'packages/data/audit': { kind: 'none', reason: 'The audit service records tool calls and session events into an immutable SQLite store for compliance; it registers nothing model-facing.' },
  'packages/embedder/embedder': { kind: 'none', reason: 'The abstract embedder seam produces retrieval-similarity vectors and rerank scores for retrieval providers; it registers no prompt, tool, schema, or session event, and vector outputs never enter model context.' },
  'packages/embedder/embedder-fakehash': { kind: 'none', reason: 'The deterministic sha256 hash-vector stub and token-recall reranker load no model and register no prompt, tool, schema, or session event; their vector and score outputs never enter model context.' },
  'packages/embedder/embedder-http': { kind: 'none', reason: 'The HTTP embedder/reranker provider relays embedding and rerank calls to an external OpenAI-compatible endpoint and registers no prompt, tool, schema, or session event; vector and rerank outputs serve retrieval similarity, not model context.' },
  'packages/query/query': { kind: 'indirect', reason: 'The abstract query-engine seam registers no prompt, tool, or session event; the deferred tool-query Consumer exposes query-execution outcomes as the model-visible tool surface.' },
  'packages/query/query-maxcompute': { kind: 'indirect', reason: 'The provider executes SQL and returns QueryOutcome over a stdio sidecar; the consuming nl2sql engine owns every model-facing result a query yields.' },
  'packages/retrieval/retrieval-inproc': { kind: 'indirect', reason: 'The in-process provider feeds fused retrieval hits through the ctx.retrieval seam to search_data_sources and the nl2sql schema-linking pipeline; it exposes no model-facing tool itself.' },
  // Data-agent tool and service packages (fork-added); model integration lives in the nl2sql engine LLM adapter.
  'packages/data/patrol-mode': { kind: 'indirect', reason: 'Pure patrol-loop operation; model integration lives in @deepseek-ai/dsh-nl2sql-engine\'s LLM adapter.' },
  'packages/data/preset-autojoin': { kind: 'indirect', reason: 'Pure preset-autojoin operation; model integration lives in @deepseek-ai/dsh-nl2sql-engine\'s LLM adapter.' },
  'packages/data/result-cache-memory': { kind: 'indirect', reason: 'Pure result-cache-memory operation; model integration lives in @deepseek-ai/dsh-nl2sql-engine\'s LLM adapter.' },
  'packages/data/scope-registry': { kind: 'indirect', reason: 'Pure scope-registry operation; model integration lives in @deepseek-ai/dsh-nl2sql-engine\'s LLM adapter.' },
  'packages/data/tool-compute': { kind: 'indirect', reason: 'Pure compute-tool operation; model integration lives in @deepseek-ai/dsh-nl2sql-engine\'s LLM adapter.' },
  'packages/data/tool-critique-sql': { kind: 'indirect', reason: 'Pure sql-critique-tool operation; model integration lives in @deepseek-ai/dsh-nl2sql-engine\'s LLM adapter.' },
  'packages/data/tool-evaluate-sql-quality': { kind: 'indirect', reason: 'Pure sql-quality-eval operation; model integration lives in @deepseek-ai/dsh-nl2sql-engine\'s LLM adapter.' },
  'packages/data/tool-present-decomposition': { kind: 'indirect', reason: 'Pure decomposition-presentation operation; model integration lives in @deepseek-ai/dsh-nl2sql-engine\'s LLM adapter.' },
  'packages/data/tool-present-table': { kind: 'indirect', reason: 'Pure table-presentation operation; model integration lives in @deepseek-ai/dsh-nl2sql-engine\'s LLM adapter.' },
  'packages/data/tool-reachability-delta': { kind: 'indirect', reason: 'Pure reachability-delta operation; model integration lives in @deepseek-ai/dsh-nl2sql-engine\'s LLM adapter.' },
  'packages/data/tool-suggest-followups': { kind: 'indirect', reason: 'Pure followup-suggestion operation; model integration lives in @deepseek-ai/dsh-nl2sql-engine\'s LLM adapter.' },
  'packages/data/tool-trigger-eval': { kind: 'indirect', reason: 'Pure eval-trigger operation; model integration lives in @deepseek-ai/dsh-nl2sql-engine\'s LLM adapter.' },
  'packages/client/ui-present-decomposition': { kind: 'indirect', reason: 'Pure decomposition-presentation UI operation; model integration lives in @deepseek-ai/dsh-nl2sql-engine\'s LLM adapter.' },
  'packages/client/ui-semantic-layer': { kind: 'indirect', reason: 'Pure semantic-layer UI operation; model integration lives in @deepseek-ai/dsh-nl2sql-engine\'s LLM adapter.' },
  'packages/client/ui-suggest-followups': { kind: 'indirect', reason: 'Pure followup-suggestion UI operation; model integration lives in @deepseek-ai/dsh-nl2sql-engine\'s LLM adapter.' },
  'packages/eval/eval-cli': { kind: 'indirect', reason: 'Pure eval-CLI operation; model integration lives in @deepseek-ai/dsh-nl2sql-engine\'s LLM adapter.' },
  'packages/eval/eval-runner-service': { kind: 'indirect', reason: 'Pure eval-runner operation; model integration lives in @deepseek-ai/dsh-nl2sql-engine\'s LLM adapter.' },
  'packages/goal/goal-eval-context': { kind: 'indirect', reason: 'Pure eval-context operation; model integration lives in @deepseek-ai/dsh-nl2sql-engine\'s LLM adapter.' },
  'packages/goal/goal-eval-policy': { kind: 'indirect', reason: 'Pure eval-policy operation; model integration lives in @deepseek-ai/dsh-nl2sql-engine\'s LLM adapter.' },
  // Section-A packages (missing Model Experience heading) added to allowlist.
  'packages/client/ui-context-layer': { kind: 'none', reason: 'Browser-side context layer surface; registers nothing model-facing.' },
  ...
  'packages/data/tool-discover-relations': { kind: 'indirect', reason: 'Pure relation-discovery operation; model integration lives in @deepseek-ai/dsh-nl2sql-engine\'s LLM adapter.' },
  'packages/data/tool-edit-definition': { kind: 'indirect', reason: 'Pure definition-edit operation; model integration lives in @deepseek-ai/dsh-nl2sql-engine\'s LLM adapter.' },
  'packages/data/tool-get-coverage': { kind: 'indirect', reason: 'Pure coverage-query operation; model integration lives in @deepseek-ai/dsh-nl2sql-engine\'s LLM adapter.' },
  'packages/data/tool-get-definition': { kind: 'indirect', reason: 'Pure definition-query operation; model integration lives in @deepseek-ai/dsh-nl2sql-engine\'s LLM adapter.' },
  'packages/data/tool-list-domains': { kind: 'indirect', reason: 'Pure domain-list operation; model integration lives in @deepseek-ai/dsh-nl2sql-engine\'s LLM adapter.' },
  'packages/data/tool-present-clarification': { kind: 'indirect', reason: 'Pure clarification-presentation operation; model integration lives in @deepseek-ai/dsh-nl2sql-engine\'s LLM adapter.' },
  'packages/data/tool-resolve-term': { kind: 'indirect', reason: 'Pure term-resolution operation; model integration lives in @deepseek-ai/dsh-nl2sql-engine\'s LLM adapter.' },
  'packages/data/tool-revert-edit': { kind: 'indirect', reason: 'Pure edit-revert operation; model integration lives in @deepseek-ai/dsh-nl2sql-engine\'s LLM adapter.' },
  'packages/data/tool-scope-routing': { kind: 'indirect', reason: 'Pure scope-routing operation; model integration lives in @deepseek-ai/dsh-nl2sql-engine\'s LLM adapter.' },
  'packages/data/tool-search-schema': { kind: 'indirect', reason: 'Pure schema-search operation; model integration lives in @deepseek-ai/dsh-nl2sql-engine\'s LLM adapter.' },
  'packages/data/tool-update-table-config': { kind: 'indirect', reason: 'Pure table-config-update operation; model integration lives in @deepseek-ai/dsh-nl2sql-engine\'s LLM adapter.' },
  'packages/eval/eval-runner': { kind: 'none', reason: 'Model-agnostic eval runner; the host injects the responder, executor, and judge collaborators, so the package registers nothing model-facing.' },
  'packages/eval/retrieval-experiment': { kind: 'indirect', reason: 'Pure retrieval-experiment operation; model integration lives in @deepseek-ai/dsh-nl2sql-engine\'s LLM adapter.' },
  'packages/query/query-postgres': { kind: 'indirect', reason: 'The provider executes SQL and returns QueryOutcome over a Postgres connection; the consuming nl2sql engine owns every model-facing result a query yields.' },
  'packages/code-runtime/code-runtime-data-python': { kind: 'indirect', reason: 'Pure data-python-runtime operation; model integration lives in @deepseek-ai/dsh-nl2sql-engine\'s LLM adapter.' },
}
